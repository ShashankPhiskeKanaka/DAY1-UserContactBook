...
//         const user = await prisma.contact.findUnique({ where : { id, deletedAt : null } });
//         return user ?? <baseContact>{};
//     }

//     checkExistence = async ( email : string ) : Promise<baseContact> => {
//         const user = await prisma.contact.findFirst({ where : { email , deletedAt : null } });
//         return user ?? <baseContact>{};
//     }
//     // fetches all users
...
//         const user = await prisma.contact.update({ where : { id : data.id }, data : {
//             name : data?.name,
...
//         const user = await prisma.contact.update({ where : { id, deletedAt : null }, data : {
//             deletedAt : new Date()
//         } })
//         return user;
//     }

//     getDeleted = async () : Promise<baseContact[]> => {
//         const contacts = await prisma.contact.findMany({ where : { deletedAt : { not : null } } })
//         return contacts;
//     }
